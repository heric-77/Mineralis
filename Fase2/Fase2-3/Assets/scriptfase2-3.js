// ═══════════════════════════════════════════════════════════════
//  MINERALIS  –  Fase 2.3  ·  O Metal que o Lago Guardou
//  scriptfase2-3.js
//  Lago Superior, Michigan — Povos Anishinaabe — 7.000 a.C.
// ═══════════════════════════════════════════════════════════════
...
const ASSETS = [
  ['bg01','Assets/Fase 2.3 - Cena 01.svg'],
  ['bg02','Assets/Fase 2.3 - Cena 02.svg'],
  ['bg03','Assets/Fase 2.3 - Cena 03.svg'],
  ['bg04','Assets/Fase 2.3 - Cena 04.svg'],
...
const BUBBLE = {
  ...
  draw(player) {
    ...
    ctx.strokeStyle='rgba(200,100,30,0.5)'; ctx.lineWidth=1.5; ctx.strokeRect(fx,fy,faceW,faceH);
    ...
    ctx.fillStyle=`rgba(200,100,30,${pulse})`; ctx.font='12px "Courier New"';
...
class Col {
  ...
  draw(){
    ...
    if(this.type==='cobre'){
      ...
      ctx.fillStyle='#888898';ctx.beginPath();ctx.ellipse(0,2,12,9,0,0,Math.PI*2);ctx.fill();
      ctx.fillStyle='#6a6a7a';ctx.beginPath();ctx.ellipse(-2,0,10,8,0,0,Math.PI*2);ctx.fill();
      ctx.strokeStyle='#555566';ctx.lineWidth=1.5;ctx.beginPath();ctx.ellipse(0,2,12,9,0,0,Math.PI*2);ctx.stroke();
    } else if(this.type==='martelo'){
      // Martelo de pedra — menor, mais angular
      ctx.fillStyle='#7a7a8a'; ctx.fillRect(-10,-6,20,12);
      ctx.fillStyle='#5a5a6a'; ctx.fillRect(-12,-8,6,16);
      ctx.strokeStyle='#404050'; ctx.lineWidth=1.5; ctx.strokeRect(-10,-6,20,12);
...
function buildL1(){
  ...
  const triggers=[
    new Trigger(2880,FL-300,200,300,'Aproximar da Águia',(player,level)=>{
      if(!player.items.includes('martelo')){notify('Colete o Martelo de Pedra primeiro!');return;}
  ...
  return{
    ...
    hint:'Colete 🪨 Maço e 🪓 Martelo, depois encontre a Águia Careca!',
    ...
    intro:[
      ...
      'Colete as ferramentas nativas e encontre a Águia Careca sagrada!'
    ],
    update(player){ tickMoving(this.plats);tickTrapdoors(this.plats); for(const e of this.enemies)e.update(this.plats,player); if(!G.dialog)this.aguia.update(player); for(const c of this.cols)c.tick(); POPUP.tick(); },
    draw(player){
      // Névoa sobre o lago
      ...
      // Águia
      this.aguia.draw();
      // Pedra onde a águia pousará
      const rx=AGUIA_ROCK_X-cam.x, ry=AGUIA_ROCK_Y-cam.y;
      if(rx>-50&&rx<W+50){
        ctx.fillStyle='#2a2a30';ctx.beginPath();ctx.ellipse(rx+30,ry+20,60,20,0,0,Math.PI*2);ctx.fill();
        ctx.fillStyle='#3a3a42';ctx.beginPath();ctx.ellipse(rx+28,ry+8,50,16,0,0,Math.PI*2);ctx.fill();
      }
...
function buildL2(){
  ...
  const triggers=[
    new Trigger(2870,FL-300,200,300,'Examinar Veio de Cobre',(player,level)=>{
      ...
      showDialog([
        '"O que vejo aqui é algo raro no planeta: cobre nativo — cobre puro, não ligado a outros minerais em um minério, pronto para ser trabalhado."',
        '"Não é necessário fundição. A natureza já fez o trabalho de purificação. Por isso este lago foi tão especial para os povos que viveram aqui."',
        '"O veio cobre penetra no basalto como raízes de uma árvore. Para extraí-lo, preciso de ritmo — não de força bruta."',
        'O afloramento revela fragmentos de cobre puro. Cena 3 desbloqueada!',
      ],()=>{notify('✦ Geologia compreendida! Vá para a extração!');level.triggers[0].done=true;setTimeout(()=>G.nextLevel(),4000);});
    }),
  ];
  return{
    ...
    intro:[
      '"O que vejo aqui é algo raro no planeta: cobre nativo — cobre puro em estado natural, sem necessidade de fundição."',
      '"Não é necessário fundição. A natureza já fez o trabalho de purificação. Por isso este lago foi tão especial para os povos que viveram aqui."',
      'Colete fragmentos de cobre nativo e examine o afloramento!'
    ],
    update(player){
...
function buildL3(){
  ...
  const triggers=[
    new Trigger(3400,FL-480,200,480,'Coletar o Gorget',(player,level)=>{
      ...
      ],()=>{notify('✦ Gorget encontrado! Vá à conclusão!');level.triggers[0].done=true;setTimeout(()=>G.nextLevel(),4000);});
    }),
  ];
  return{
  ...
  const triggers=[
    new Trigger(2780,FL-460,240,460,'Completar a Fase!',(player,level)=>{
      ...
      showDialog([
        ...
        `🏆 FASE 2.3 CONCLUÍDA! Cobre coletado: ${cobreN} fragmentos.\nDescoberta: a mineração nativa que o mundo esqueceu.\nPróximo destino: Europa Medieval!`,
...
function drawHUD(player,level){
  ...
  ctx.fillStyle='rgba(200,120,40,.9)';ctx.font='13px "Courier New"';
  ctx.textAlign='center';ctx.fillText(level.title,W/2,24);ctx.textAlign='left';
  ctx.fillStyle='#e08830';ctx.font='bold 15px "Courier New"';
  ...
  ctx.strokeStyle=player.activeTool?'#e08830':'#444';ctx.lineWidth=1.5;_roundRect(16,tY,140,28,4);ctx.stroke();
  if(player.activeTool&&ITEM_DEFS[player.activeTool]){
    ...
    ctx.font='11px "Courier New"';ctx.fillStyle='#e08830';ctx.fillText(def.nome,42,tY+20);
  } else {ctx.font='11px "Courier New"';ctx.fillStyle='#555';ctx.fillText('Sem ferramenta',22,tY+20);}
  // Botão inventário
  ctx.fillStyle='rgba(200,100,40,0.15)';_roundRect(162,tY,46,28,4);ctx.fill();
  ctx.strokeStyle='#8a4820';ctx.lineWidth=1.5;_roundRect(162,tY,46,28,4);ctx.stroke();
  ctx.font='bold 11px "Courier New"';ctx.fillStyle='#c06030';ctx.textAlign='center';ctx.fillText('[I]',185,tY+19);ctx.textAlign='left';
  // Itens coletados no HUD
  let ix=W-16;const inv=[];
  if(player.items.includes('gorget')) inv.push('🌐 GORGET');
  if(player.items.includes('escopro')) inv.push('🔧 ESCOPRO');
  if(player.items.includes('martelo')) inv.push('🪓 MARTELO');
  if(player.items.includes('maco'))    inv.push('🪨 MAÇO');
  const cn=player.items.filter(i=>i==='cobre').length;
  if(cn>0) inv.push('🟠 '+cn);
  for(const it of inv){ctx.fillStyle='#e08830';ctx.font='12px "Courier New"';ctx.textAlign='right';ctx.fillText(it,ix,tY+20);ctx.textAlign='left';ix-=ctx.measureText(it).width+20;}
  // Hint
  ctx.fillStyle='rgba(220,180,100,.72)';ctx.font='12px "Courier New"';
  ctx.textAlign='center';ctx.fillText(level.hint,W/2,H-10);ctx.textAlign='left';
  if(G.timeOnLevel<600){
    ctx.fillStyle='rgba(0,0,0,0.55)';ctx.fillRect(8,H-44,460,28);
    ctx.fillStyle='#aaa';ctx.font='12px "Courier New"';
    ctx.fillText('← → Mover   ↑/Espaço Pular   E Interagir   [Stomp inimigo]   I Inventário',14,H-25);
  }
}
...
function drawTitle(){
  ...
  ctx.fillText('[I] Inventário   [M] Menu Principal',W/2,538);
  ctx.textAlign='left';
}
...
function _journalColetar(tipo, count) {
  ...
  try {
    ...
    // Global diary — store count for multiples
    if (count && count > 1) {
    ...
    // Phase-specific save
    if (!save.fases) save.fases = {};
    if (!save.fases['2.3']) save.fases['2.3'] = {desbloqueada:true, estrelas:0, coletados:{}};
    if (!save.fases['2.3'].coletados) save.fases['2.3'].coletados = {};
    if (count && count > 1) {
      save.fases['2.3'].coletados[id] = Math.max(Number(save.fases['2.3'].coletados[id])||0, count);
    } else {
      save.fases['2.3'].coletados[id] = true;
    }
...
if(!gameReady){
  (function loadLoop(){
    ...
    ctx.fillStyle='#050808';ctx.fillRect(0,0,W,H);
    ctx.fillStyle='#e08040';ctx.font='bold 22px "Courier New"';ctx.textAlign='center';
    ...
    ctx.fillText('Fase 2.3 — O Metal que o Lago Guardou',W/2,H/2+36);
    ctx.textAlign='left';
  })();
}
