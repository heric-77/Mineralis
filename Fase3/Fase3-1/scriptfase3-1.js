...
const INV={
  ...
  draw(player){
    ...
    if(items.length===0){
      ...
      if(scrollTop>0){ctx.font='bold 11px "Courier New"';ctx.fillStyle='#d0a8e0';ctx.textAlign='center';ctx.fillText('▲ mais',PX+16+COL_W/2,CY+8);ctx.textAlign='left';}
      if(scrollTop+maxRows<items.length){ctx.font='bold 11px "Courier New"';ctx.fillStyle='#d0a8e0';ctx.textAlign='center';ctx.fillText('▼ mais',PX+16+COL_W/2,CY+16+maxRows*ROW_H+2);ctx.textAlign='left';}
