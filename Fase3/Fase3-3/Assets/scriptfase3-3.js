// ═══════════════════════════════════════════════════════════════
//  MINERALIS  –  Fase 3.3  ·  O Vermelho que Move o Mundo
//  scriptfase3-3.js
//  Almadén, La Mancha, Espanha — Século XVI
//  Cinábrio · Mercúrio · Vapor Invisível
// ═══════════════════════════════════════════════════════════════
...
const ASSETS=[
  ['bg01','Assets/Fase 3.3 - Cena 01.svg'],
  ['bg02','Assets/Fase 3.3 - Cena 02.svg'],
  ['bg03','Assets/Fase 3.3 - Cena 03.svg'],
  ['bg04','Assets/Fase 3.3 - Cena 04.svg'],
  ['lince','Assets/lince_iberico.svg'],
  ['cornue','Assets/destilador_cornue.svg'],
  ['picareta_img','Assets/picareta_calcario.svg'],
  ['tocha_img','Assets/tocha_alcatrao.svg'],
  ['frasco_img','Assets/frasco_mercurio.svg'],
];
...
const BUBBLE={
  ...
  slowMode:false, // intoxicação por mercúrio — texto aparece mais devagar
  show(messages,cb,speaker='CORVAN',slow=false){
    this.queue=[...messages];this.cb=cb;this.active=true;
    this.speakerTxt=speaker;this.slowMode=slow;G.dialog=true;this._next();
  },
  _next(){
  ...
  draw(player){
    ...
    // Em modo slow (intoxicado), borda violeta
    const borderCol=this.slowMode?'#6040c0':'#c02010';
    ctx.shadowColor='rgba(0,0,0,0.6)';ctx.shadowBlur=12;
    ctx.fillStyle='rgba(4,0,0,0.95)';roundRect(bx,by,bubW,bubH,14);ctx.fill();ctx.shadowBlur=0;
    ctx.strokeStyle=borderCol;ctx.lineWidth=2.5;roundRect(bx,by,bubW,bubH,14);ctx.stroke();
    ...
    ctx.strokeStyle='rgba(180,40,20,0.5)';ctx.lineWidth=1.5;ctx.strokeRect(fx,fy,faceW,faceH);
    const tx=fx+faceW+pad;
    ctx.font=NAMEFNT;ctx.fillStyle=this.slowMode?'#9070d0':'#e04030';ctx.fillText(this.speakerTxt,tx,by+pad+14);
    ctx.font=FONT;ctx.fillStyle=this.slowMode?'#c0b0e0':'#f0e8c0';
    this.lines.forEach((l,i)=>ctx.fillText(l,tx,by+pad+36+i*lineH));
    const pulse=0.55+Math.sin(Date.now()/400)*0.45;
    ctx.fillStyle=`rgba(180,40,20,${pulse})`;ctx.font='12px "Courier New"';
...
function drawNotif(){
  ...
  ctx.fillStyle='#e04030';ctx.textAlign='center';ctx.fillText(notifText,W/2,ny+19);ctx.textAlign='left';
  ctx.restore();
}
...
const POPUP={
  ...
  draw(){
    ...
    ctx.font='bold 13px "Courier New"';ctx.fillStyle='#e04030';ctx.fillText(this.title,PX+60,PY+28);
...
class LinceIberico{
  ...
  draw(){
    ...
    if(IMG['lince']&&IMG['lince'].complete&&IMG['lince'].naturalWidth>0){
      const dw=80,dh=80;
      const flipX=Math.cos(this.angle)<0;
...
class Trigger{
  ...
  draw(px,py){
    ...
    ctx.fillStyle='#e04030';ctx.textAlign='center';ctx.fillText(txt,sx,sy);ctx.textAlign='left';
  }
}
...
class Player{
  ...
  update(level){
    ...
    if(this.vaporExposure>=100&&!G.dialog){
      ...
      // Retrocede ao último checkpoint da cena
      if(level.checkpoint){ this.x=level.checkpoint.x;this.y=level.checkpoint.y; }
      showDialog([
      ...
      ],()=>{this.vaporExposure=30;},null,true);
    }
    ...
    if(isE()&&this.items.includes('destilador')&&level.vaporZones){
      for(const vz of level.vaporZones){
        if(!vz.cleared&&vz.overlaps(this.x,this.y,this.w,this.h)){
          ...
          this.vaporExposure=Math.max(0,this.vaporExposure-30);
          break;
        }
      }
    }
...
function buildL1(){
  ...
  const LINCE_X=3000,LINCE_Y=FL-90;
  ...
  const triggers=[
    new Trigger(2880,FL-300,200,300,'Aproximar do Lince',(player,level)=>{
      ...
      level.lince.land(LINCE_X,LINCE_Y);
      showDialog([
  ...
  return{
    ...
    hint:'Colete ⛏ Picareta e 🔥 Tocha, depois encontre o Lince Ibérico!',
...
function drawHUD(player,level){
  ...
  ctx.fillStyle='rgba(200,40,20,.9)';ctx.font='13px "Courier New"';
  ctx.textAlign='center';ctx.fillText(level.title,W/2,24);ctx.textAlign='left';
  ctx.fillStyle='#e04030';ctx.font='bold 15px "Courier New"';
  ...
  if(player.activeTool&&ITEM_DEFS[player.activeTool]){
    ...
    ctx.font='11px "Courier New"';ctx.fillStyle='#e04030';ctx.fillText(def.nome,42,tY+20);
  } else {ctx.font='11px "Courier New"';ctx.fillStyle='#555';ctx.fillText('Sem ferramenta',22,tY+20);}
  // Inventário
  ctx.fillStyle='rgba(180,40,20,0.15)';_rr(162,tY,46,28,4);ctx.fill();
  ctx.strokeStyle='#8a2010';ctx.lineWidth=1.5;_rr(162,tY,46,28,4);ctx.stroke();
  ...
  if(player.items.includes('frasco'))     inv.push('⚗️ FRASCO');
  if(player.items.includes('destilador')) inv.push('🧪 CORNUE');
  if(player.items.includes('tocha'))      inv.push('🔥 TOCHA');
  if(player.items.includes('picareta_calc')) inv.push('⛏ PICARETA');
  const cn=player.items.filter(i=>i==='cinabrio').length;
  if(cn>0) inv.push('🔴 '+cn);
  ...
  if(player.vaporExposure>15){
    const pct=player.vaporExposure/100,bW=150,bX=16,bY=tY+32;
    const col=player.torchBlue>0.5?`rgba(100,80,255,0.9)`:`rgba(180,40,20,0.9)`;
    ctx.fillStyle='rgba(0,0,0,0.6)';ctx.fillRect(bX,bY,bW,12);
    ctx.fillStyle=col;ctx.fillRect(bX,bY,bW*pct,12);
    ctx.strokeStyle='rgba(200,100,100,0.6)';ctx.lineWidth=1;ctx.strokeRect(bX,bY,bW,12);
    ctx.font='10px "Courier New"';ctx.fillStyle='#ffb0a0';
    ctx.fillText(player.torchBlue>0.3?'⚠ VAPOR Hg':'EXPOSIÇÃO',bX+2,bY+10);
  }
  // Hint
  ctx.fillStyle='rgba(220,140,120,.72)';ctx.font='12px "Courier New"';
  ctx.textAlign='center';ctx.fillText(level.hint,W/2,H-10);ctx.textAlign='left';
  if(G.timeOnLevel<600){
    ctx.fillStyle='rgba(0,0,0,0.55)';ctx.fillRect(8,H-44,480,28);
    ctx.fillStyle='#aaa';ctx.font='12px "Courier New"';
    ctx.fillText('← → Mover  ↑/Espaço Pular  E Interagir/Destilador  I Inventário',14,H-25);
  }
}
...
function drawTitle(){
  const bg=IMG['bg04'];
  if(bg&&bg.complete&&bg.naturalWidth>0){ctx.globalAlpha=0.55;drawBg('bg04');ctx.globalAlpha=1;}
  else{const g=ctx.createLinearGradient(0,0,0,H);g.addColorStop(0,'#200408');g.addColorStop(1,'#060002');ctx.fillStyle=g;ctx.fillRect(0,0,W,H);}
  ...
  // Lince andando
  if(IMG['lince']&&IMG['lince'].complete){
    const lx=((Date.now()/22)%(W+120))-60;const ly=200+Math.sin(Date.now()/900)*18;
    ctx.save();ctx.translate(lx+50,ly);ctx.scale(-1,1);ctx.drawImage(IMG['lince'],0,0,96,96,-50,-50,100,100);ctx.restore();
  }
  ctx.textAlign='center';
  ctx.shadowColor='#c02010';ctx.shadowBlur=40;
  ctx.fillStyle='#e03020';ctx.font='bold 48px "Courier New"';ctx.fillText('O VERMELHO QUE MOVE O MUNDO',W/2,170);
  ctx.shadowBlur=0;
  ctx.fillStyle='#a05040';ctx.font='22px "Courier New"';ctx.fillText('Fase 3.3  —  Almadén, Espanha · Século XVI',W/2,218);
  if(IMG.card33){
  ...
  ctx.fillStyle=`rgba(200,60,40,${.55+Math.sin(Date.now()/550)*.4})`;ctx.font='19px "Courier New"';
  ctx.fillText('▶  Pressione ENTER para começar  ◀',W/2,460);
  ctx.fillStyle='#c0c8d8';ctx.font='18px "Courier New"';
  ctx.fillText('← → Mover  ↑/Espaço Pular  E Interagir/Destilador  I Inventário',W/2,504);
