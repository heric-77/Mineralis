...
function solid(x,y,w,h){return{x,y,w,h,type:'solid'};}function spike(x,y,w){return{x,y,w,h:20,type:'spike'};}function trap(x,y,w){return{x,y,w,h:16,type:'trap',t:0,open:false};}function movH(x,y,w,min,max,v){return{x,y,w,h:18,type:'movH',min,max,vx:v};}function bridge(x,y,w){return{x,y,w,h:10,type:'bridge'};}
...
function drawPlatform(p){if(p.type==='trap'&&p.open)return;const x=p.x-cam.x,y=p.y-cam.y;if(x>W+80||x+p.w<-80)return;if(p.type==='spike'){ctx.fillStyle='#48351c';for(let i=0;i<p.w;i+=20){ctx.beginPath();ctx.moveTo(x+i,y+20);ctx.lineTo(x+i+10,y);ctx.lineTo(x+i+20,y+20);ctx.closePath();ctx.fill();}return;}if(p.type==='bridge'){ctx.fillStyle='#8a5a2c';ctx.fillRect(x,y,p.w,p.h);ctx.fillStyle='#6b4420';for(let i=6;i<p.w;i+=14)ctx.fillRect(x+i,y,2,p.h);ctx.fillStyle='rgba(216,179,74,.28)';ctx.fillRect(x,y,p.w,2);ctx.fillStyle='#5c3a1a';ctx.fillRect(x-3,y-14,4,14);ctx.fillRect(x+p.w-1,y-14,4,14);ctx.fillStyle='#8a5a2c';ctx.fillRect(x-4,y-16,p.w+8,3);ctx.fillStyle='#5c3a1a';ctx.fillRect(x+6,y+p.h,4,42);ctx.fillRect(x+p.w-10,y+p.h,4,42);return;}ctx.fillStyle=p.type==='movH'?'#3d3b28':'#273018';ctx.fillRect(x,y,p.w,p.h);ctx.fillStyle='rgba(216,179,74,.22)';ctx.fillRect(x,y,p.w,2);ctx.strokeStyle='rgba(0,0,0,.35)';ctx.strokeRect(x,y,p.w,p.h);}
...
class Trigger{constructor(x,y,w,h,label,fn,auto=false){this.x=x;this.y=y;this.w=w;this.h=h;this.label=label;this.fn=fn;this.done=false;this.auto=auto;}draw(px,py){if(this.done||this.auto)return;const near=Math.abs(px+18-(this.x+this.w/2))<this.w/2+70&&Math.abs(py+35-(this.y+this.h/2))<this.h/2+70;if(!near)return;const sx=this.x+this.w/2-cam.x,sy=this.y-cam.y-18;ctx.fillStyle='rgba(0,0,0,.82)';ctx.font='bold 13px Courier New';const txt='[E] '+this.label,tw=ctx.measureText(txt).width+22;const boxTop=Math.min(sy-17,py-cam.y-32);roundRect(sx-tw/2,boxTop,tw,24,5);ctx.fill();ctx.strokeStyle=UI_ACCENT;ctx.stroke();ctx.fillStyle=UI_ACCENT;ctx.textAlign='center';ctx.fillText(txt,sx,boxTop+17);ctx.textAlign='left';}}
...
function drawHUD(p,l){
  ...
  const TOOL_DEFS=[
    {id:'bateia_madeira',  icon:'🥣',nome:'Bateia Curvada'},
    {id:'calabaca_agua',   icon:'💧',nome:'Calabaça'},
    {id:'bastao_sombra',   icon:'☀️',nome:'Bastão Sombra'},
  ];
  // Mapa de nomes curtos para o painel (evita overflow). "Bateia Curvada" (não
  // apenas "Bateia") para não ser confundida com a Bateia do Garimpo de Serra
  // Pelada (Fase 1.2), que também aparece no Diário de Bordo compartilhado.
  const NOME_CURTO={bateia_madeira:'Bateia Curvada',calabaca_agua:'Calabaça',bastao_sombra:'Bastão'};
  ...
  const tools=TOOL_DEFS.filter(t=>p.items.includes(t.id)&&t.id!==p.activeTool);
...
function drawDeath(){
  ...
  const msgs={corrente:'A CORRENTE TE LEVOU!',queda:'CORVAN CAIU!',rocha:'TERRENO PERIGOSO!',inimigo:'O PLANALTO VINGOU-SE!'};
  const subs={corrente:'O Mutirikwe é generoso com seus filhos — mas fatal com os descuidados!',queda:'Os kopjes de granito do Grande Zimbábue não têm misericórdia.',rocha:'As rochas de granito do planalto são traiçoeiras. Cuidado!',inimigo:'Os guardiões do Grande Zimbábue não perdoam intrusos!'};
...
function drawComplete(){drawBg('bg04');ctx.fillStyle='rgba(0,0,0,.62)';ctx.fillRect(0,0,W,H);ctx.textAlign='center';ctx.shadowColor=UI_ACCENT;ctx.shadowBlur=38;ctx.fillStyle=UI_ACCENT;ctx.font='bold 42px Courier New';ctx.fillText('✦  FASE 4.2 CONCLUÍDA  ✦',W/2,118);ctx.shadowBlur=0;drawCorvan(W/2-40,140,3,false,Date.now()/260,'bastao_sombra');ctx.fillStyle='#e8dba5';ctx.font='17px Courier New';ctx.fillText('O ouro revelou o comércio e a memória do Grande Zimbabué.',W/2,340);const lines=['🥣 Bateia de Madeira - garimpo Shona sem ferramentas metálicas','🟡 Ouro Aluvial - pepitas densas que afundam na bateia','✧ Mica Dourada - brilho enganoso, leve e flexível','🐦 Pássaro de Esteatita - ancestralidade, símbolo e resistência'];ctx.fillStyle='#d8c891';ctx.font='14px Courier New';lines.forEach((l,i)=>ctx.fillText(l,W/2,400+i*32));const _scoreY=400+lines.length*32+40;ctx.fillStyle='#c8c0a0';ctx.font='16px Courier New';ctx.fillText(`Pontuação: ◈ ${G.player?.score||0}   Mortes: ${G.deaths}`,W/2,_scoreY);ctx.fillStyle=`rgba(216,179,74,${.6+Math.sin(Date.now()/550)*.35})`;ctx.font='15px Courier New';ctx.fillText('✦ Fase 4.3 desbloqueada!   [M] Menu Principal',W/2,_scoreY+35);ctx.textAlign='left';}

function buildL1(){const FL=590,WW=3100,WH=900;const plats=basePlats(FL,WW,WH);const cols=[new Col(210,FL-55,'bateia_madeira'),new Col(540,FL-55,'calabaca_agua'),new Col(1350,FL-225,'mica_dourada')];const aguia={x:2520,y:FL-215,gifted:false,vx:0.6,patrolMin:2200,patrolMax:2850,t:0};const triggers=[new Trigger(2880,FL-190,120,190,'Descer ao rio',(p,l)=>{if(!p.items.includes('bateia_madeira')){notify('Pegue a Bateia de Madeira primeiro.');return;}if(!p.items.includes('calabaca_agua')){notify('Pegue a Calabaça de Água primeiro.');return;}if(!p.items.includes('bastao_sombra')){notify('Interaja com o Peixe-Águia para receber o Bastão de Sombra.');return;}l.triggers[0].done=true;sfx('unlock');showDialog(['Século XIII. Diante de nós está o Grande Zimbabué: muralhas de granito sem argamassa e uma torre cônica que observa o planalto.','Mais de 18 mil pessoas viveram aqui. O ouro viajava do interior da África até Sofala, depois para a Índia e além.','A primeira tarefa é simples: recolher as ferramentas Shona e seguir para o rio Mutirikwe.'],()=>setTimeout(()=>G.nextLevel(),600));},true)];return{id:1,bg:'bg01',W:WW,H:WH,startX:60,startY:FL-90,title:'Grande Zimbabué - Muralhas ao Amanhecer',hint:p=>!p.items.includes('bateia_madeira')?'🥣 Pegue a Bateia de Madeira →':!p.items.includes('calabaca_agua')?'💧 Pegue a Calabaça de Água →':!p.items.includes('bastao_sombra')?'🦅 Encontre o Peixe-Águia e receba o ☀️ Bastão de Sombra →':'✦ Ferramentas prontas - desça ao rio →',plats,cols,triggers,aguia,intro:['Capim dourado, kopjes de granito e muralhas curvas no amanhecer africano.','Colete a Bateia de Madeira e a Calabaça. Depois procure o Peixe-Águia sobre a rocha acima do rio.'],update(p){
...
function buildL3(){const FL=590,WW=3400,WH=900;
  ...
  // basePlats deixa vãos entre todos os blocos de piso. Em cada vão, colocamos
  // uma pontezinha de tábuas por cima (com corrimão e pilares) e deixamos a
  // correnteza visível correndo por baixo dela — igual em toda a cena, Corvan
  // sempre atravessa em segurança por cima, sem precisar da Calabaça aqui.
  const GAPS=[[340,80],[650,70],[940,70],[1230,60],[1530,60],[1810,70],[2110,60],[2410,70]];
  const currentsBridge=GAPS.map(([gx,gw])=>{
    plats.push(bridge(gx,FL,gw));
    const c=new RiverCurrent(gx-8,FL+10,gw+16,40);c.safe=true;return c;
  });
  currentsBridge.forEach(b=>b.safe=true);
