...
function _voltarAoMenu(){ _salvarFase(G.player?.score||0,G.deaths); unlockPhase('5.1'); try{sessionStorage.setItem('mineralis_session','1');}catch(e){}
  window.location.href='../../MenuPrincipal/index.html?unlocked=5.1'; }

// ── Assets ───────────────────────────────────────────────────────
const IMG={};
const ASSETS=[
  ['bg01','Assets/Cena01_Caravana_Timbuktu.svg'],
  ['bg02','Assets/Cena02_Taoudenni_Sal.svg'],
  ['bg03','Assets/Cena03_Bazar_Harmattan.svg'],
  ['bg04','Assets/Cena04_Biblioteca_Sankore.svg'],
  ['dromedario_img','Assets/4_3_dromedario.svg'],
  ['astrolabio_img','Assets/4_3_astrolabio.svg'],
  ['machado_img',   'Assets/4_3_machado_pedra.svg'],
  ['balanca_img',   'Assets/4_3_balanca_bronze.svg'],
  ['manuscrito_img','Assets/4_3_manuscrito.svg'],
  ['card43',        'Assets/4_3_timbuktu.svg'],
];



let assetsLoaded=0, totalAssets=ASSETS.length, gameReady=false; // totalAssets calculado dinamicamente
...
// ── Corvan pixel-art inline — padrão fase 1-1 ────────────────────
function drawCorvanInline(cx,cy,scale=1,flipX=false,frame=0){
  const S=scale;
  ...
  // Chapéu
  r(7,3,18,2,'#3a2208');r(9,1,14,4,'#4a2e10');r(13,0,6,3,'#c8a020');r(14,0,4,2,'#ffe060');
  // Cabeça
...
const ITEM_DEFS={
  ...
  picareta_calcario:{ cat:'ferramenta',nome:'Picareta de Calcário',  icon:'⛏',fase:'3.3',journalId:'picareta_calcario_item',desc:'Adaptada para rocha sedimentar.\nRevela veias de cinábrio com precisão.' },
  // Novas
...
const BUBBLE={
  ...
  draw(player){
    ...
    ctx.fillStyle='rgba(4,3,0,0.96)';roundRect(bx,by,bubW,bubH,14);ctx.fill();ctx.shadowBlur=0;
    // Borda dupla dourada
    ctx.strokeStyle='#c89020';ctx.lineWidth=2.5;roundRect(bx,by,bubW,bubH,14);ctx.stroke();
    ctx.strokeStyle='rgba(200,150,20,0.2)';ctx.lineWidth=1;roundRect(bx+4,by+4,bubW-8,bubH-8,10);ctx.stroke();
    // Cauda
    const tailBaseX=Math.max(bx+30,Math.min(pcx,bx+bubW-30)),tailTopY=by+bubH,tailTipX=pcx,tailTipY=Math.min(pcy,tailTopY+36);
    ctx.fillStyle='rgba(4,3,0,0.96)';ctx.beginPath();ctx.moveTo(tailBaseX-14,tailTopY);ctx.lineTo(tailBaseX+14,tailTopY);ctx.lineTo(tailTipX,tailTipY);ctx.closePath();ctx.fill();
    ...
    ctx.strokeStyle='rgba(200,150,20,0.45)';ctx.lineWidth=1.5;roundRect(fx,fy,faceW,faceH,6);ctx.stroke();
    ctx.save();ctx.beginPath();roundRect(fx+1,fy+1,faceW-2,faceH-2,5);ctx.clip();
    const faceScale=faceW/18*0.82;
    drawCorvanInline(fx+faceW/2-16*faceScale,fy+4,faceScale,false,this.faceFrame*4);
    // Animação de boca falando
    const open=Math.abs(Math.sin(this.faceFrame*4))*1.8*faceScale;
    if(open>0.5){ctx.fillStyle='#2a0e06';ctx.fillRect(fx+faceW/2-16*faceScale+12*faceScale,fy+4+13*faceScale,8*faceScale,open);}
    ctx.restore();
    // Nome + separador + texto
    const tx=fx+faceW+pad;
    ctx.font=NAMEFNT;ctx.fillStyle='#e0b030';ctx.fillText(this.speakerTxt,tx,by+pad+14);
    ctx.fillStyle='rgba(200,150,20,0.35)';ctx.fillRect(tx,by+pad+20,textAreaW,1);
    ctx.font=FONT;ctx.fillStyle='#f0e8c0';this.lines.forEach((l,i)=>ctx.fillText(l,tx,by+pad+40+i*lineH));
    const pulse=0.5+Math.sin(Date.now()/400)*0.5;
    ctx.fillStyle=`rgba(200,150,20,${pulse})`;ctx.font='12px "Courier New"';ctx.textAlign='right';ctx.fillText('[E] Continuar →',bx+bubW-pad,by+bubH-10);ctx.textAlign='left';
  }
};
...
function drawNotif(){
  ...
  ctx.fillStyle='rgba(0,0,0,0.88)';roundRect(nx,ny,tw,28,6);ctx.fill();ctx.strokeStyle='#e0b030';ctx.lineWidth=1.5;roundRect(nx,ny,tw,28,6);ctx.stroke();
  ctx.fillStyle='#fff0d0';ctx.textAlign='center';ctx.fillText(notifText,W/2,ny+19);ctx.textAlign='left';ctx.restore();
}
...
class SalDeposit {
  ...
  draw(player){
    ...
    // Halo dourado de fundo para destacar do branco
    const pulse=0.5+Math.sin(Date.now()/300)*0.5;
    ctx.fillStyle=`rgba(200,170,60,${0.18+pulse*0.12})`;
    ctx.fillRect(-6,-6,this.w+12,this.h+12);
    // Camadas horizontais de halita — mais escuras para contrastar
    for(let r=0;r<4;r++){
      const clr=['#d8d2c0','#ccc6b4','#c0baa8','#b4ae9c'][r];
      ctx.fillStyle=clr; ctx.fillRect(0,r*9,this.w,9);
      ctx.strokeStyle='rgba(140,130,110,0.6)'; ctx.lineWidth=1; ctx.strokeRect(0,r*9,this.w,9);
    }
    // Brilho cristalino
    ctx.fillStyle='rgba(255,255,240,0.7)'; ctx.beginPath(); ctx.arc(12,8,4,0,Math.PI*2); ctx.fill();
...
class Dromedario{
  ...
  draw(){
    ...
    if(IMG['dromedario_img']&&IMG['dromedario_img'].complete&&IMG['dromedario_img'].naturalWidth>0){
      const dw=110,dh=110,bob=this.state==='landed'?0:Math.sin(this.frame*1.2)*5;
...
function buildL1(){
  ...
  const DROM_X=3000,DROM_Y=FL-55; // pés do dromedário no chão (dh/2=55)
  const dromedario=new Dromedario();
  dromedario.visible=true;
  dromedario.x=DROM_X; dromedario.y=DROM_Y;
  dromedario.state='landed'; // aguarda estático no final da cena
  const triggers=[
...
function buildL2(){
  ...
  // Todos no chão acessível (FL-60) — sem flutuantes inalcançáveis
  const salDeposits=[
    new SalDeposit(380,FL-60), new SalDeposit(700,FL-60), new SalDeposit(1060,FL-60),
    new SalDeposit(1400,FL-60), new SalDeposit(1900,FL-60), new SalDeposit(2400,FL-60),
  ];
...
function buildL4(){
  ...
  const dromedario=new Dromedario();
  dromedario.visible=true;
  dromedario.x=200; dromedario.y=FL-55; // começa no chão perto do Corvan
  dromedario.state='landed'; // guia — não orbita
  const dromMessages=[
  ...
  // Manuscrito no topo da escadaria (plataforma y=FL-420, dentro do mundo WW=3200)
  ...
  return{
    ...
    update(player){
      ...
      // Dromedário guia: avança suavemente ~420px à frente do Corvan no chão
      if(!G.dialog){
        const targetX=Math.min(WW-250, player.x+420);
        this.dromedario.x+=(targetX-this.dromedario.x)*0.022;
        this.dromedario.y=FL-55; // mantém no chão
        this.dromedario.frame+=0.04;
      }
...
function drawHUD(player,level){
  // Barra escura primeiro, depois corações por cima
  ctx.fillStyle='rgba(0,0,0,0.55)';ctx.fillRect(0,0,W,34);
  // Corações de vida — forma de coração
  ...
  ctx.textAlign='center';ctx.fillText(level.title,W/2,24);ctx.textAlign='left';ctx.shadowBlur=0;
  // Score — canto direito
  ...
  ctx.fillStyle='rgba(4,3,0,0.88)';_rr(PX,PY,PW,PH,6);ctx.fill();
  ctx.shadowBlur=0;
  ctx.strokeStyle='#806010';ctx.lineWidth=1.5;_rr(PX,PY,PW,PH,6);ctx.stroke();
  ctx.strokeStyle='rgba(180,140,20,0.25)';ctx.lineWidth=1;_rr(PX+3,PY+3,PW-6,PH-6,4);ctx.stroke();
  ctx.restore();
  const midX=PX+PW/2;
  const kw=26,kx=PX+PW-kw-6,ky=PY+5;
  ctx.font='11px serif';ctx.fillStyle='#b09020';ctx.textAlign='left';ctx.fillText('📔',PX+8,PY+20);
  ctx.font='bold 10px "Courier New"';ctx.fillStyle='#b09020';ctx.fillText('DIÁRIO DE BORDO',PX+24,PY+20);
  ctx.fillStyle='rgba(180,140,20,0.2)';_rr(kx,ky,kw,18,3);ctx.fill();
  ctx.strokeStyle='#b09020';ctx.lineWidth=1;_rr(kx,ky,kw,18,3);ctx.stroke();
  ctx.font='bold 10px "Courier New"';ctx.fillStyle='#e0b030';
  ...
  // Harmattan timer (mantido por ser mecânica da cena)
  if(level.harmattanActive&&!level.astroCalibrated){
    const pct=Math.max(0,level.harmattanTimer/1800);
    const bW=180,bX=W/2-90,bY=46;
    ctx.fillStyle='rgba(0,0,0,0.6)';ctx.fillRect(bX,bY,bW,14);
    ctx.fillStyle=pct>0.4?'rgba(200,140,20,0.9)':'rgba(200,60,20,0.9)';ctx.fillRect(bX,bY,bW*pct,14);
    ctx.strokeStyle='rgba(200,150,40,0.5)';ctx.lineWidth=1;ctx.strokeRect(bX,bY,bW,14);
    ctx.font='10px "Courier New"';ctx.fillStyle='#ffd060';ctx.textAlign='center';ctx.fillText('⚠ HARMATTAN',W/2,bY+11);ctx.textAlign='left';
  }
...
// ── Title Screen ──────────────────────────────────────────────────
function drawTitle(){
  // Fundo: Cena 03 (Bazar do Harmattan) — mesmo padrão da Fase 1-1
  const bg=IMG['bg03'];
  if(bg&&bg.complete&&bg.naturalWidth>0){ctx.globalAlpha=0.85;ctx.drawImage(bg,0,0,W,H);ctx.globalAlpha=1;}
  else{const g=ctx.createLinearGradient(0,0,0,H);g.addColorStop(0,'#3a2010');g.addColorStop(1,'#100800');ctx.fillStyle=g;ctx.fillRect(0,0,W,H);}
  ...
  ctx.textAlign='center';
  // Título — case normal, cor dourada, sombra, igual 1-1
  ctx.shadowColor='#e0b030';ctx.shadowBlur=40;
  ctx.fillStyle='#e0b030';ctx.font='bold 46px "Courier New"';ctx.fillText('A Cidade Onde o Ouro Vira Conhecimento',W/2,148);
  ctx.shadowBlur=0;
  ctx.fillStyle='#c8a060';ctx.font='19px "Courier New"';ctx.fillText('Fase 4.3  —  Timbuktu, Mali · Século XIV',W/2,200);
  // Card central — mesmo posicionamento da 1-1
  if(IMG.card43){
    ...
    ctx.drawImage(IMG.card43,cardX,cardY,cardSize,cardSize);
  }
  // ENTER pulsante
...
function drawDeath(){
  ...
  const msgs={queda:'CORVAN CAIU!',espinho:'QUE ESPINHO!',inimigo:'O SAARA É IMPLACÁVEL!',areia:'A TEMPESTADE DE AREIA TE CEGOU!'};
  const subs={queda:'O deserto do Saara não perdoa queda alguma.',espinho:'As armadilhas das rotas de caravana são mortais!',inimigo:'O guardião da rota de sal foi mais veloz. Pule sobre eles!',areia:'O Harmattan varreu tudo. Nunca pare durante a tempestade!'};
...
// ── Música de fundo — Fase 4.3 (Timbuktu/Saara): pentatônica maior africana ──
let _bgMusicActive=false,_bgMusicTimeout=null,_bgMusicGain=null;
const _NOTES_TIM=[261.6,293.6,329.6,392,440,523.2,587.3,659.2]; // pentatônica maior
function startBgMusic(){
  ...
  function _nota(freq,start,dur){
    ...
    g.gain.setValueAtTime(0,start);g.gain.linearRampToValueAtTime(0.07,start+0.04);
    g.gain.setValueAtTime(0.07,start+dur-0.1);g.gain.linearRampToValueAtTime(0,start+dur);
    o.connect(g);g.connect(_bgMusicGain);o.start(start);o.stop(start+dur);
  }
  const SEQ=[0,2,4,6,4,6,4,2,4,6,5,4,2,4,2,0]; // ritmo animado africano
  function _ciclo(){
    if(!_bgMusicActive)return;
    const t=AC.currentTime+0.1;
    SEQ.forEach((idx,i)=>_nota(_NOTES_TIM[idx%_NOTES_TIM.length],t+i*0.32,0.36));
    _bgMusicTimeout=setTimeout(_ciclo,(SEQ.length*0.32-0.2)*1000);
  }
  _ciclo();
}
